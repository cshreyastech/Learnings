...
SocketServer::SocketServer(const int port) :
    ...
    client_addr_size_(sizeof(client_addr_)),
    port_(port),
    sock_fdesc_conn_(0) {
}
...
void SocketServer::ConnectToNetwork() {
  ...
  if (bind(sock_fdesc_conn_, (struct sockaddr*)&server_addr_,
           server_addr_size_) == -1) {
...
const int SocketServer::ReceiveInt()
{
  ...
  num_bytes = recvfrom(sock_fdesc_conn_, data_arr, packet_size,
      0, (struct sockaddr*)&client_addr_,&client_addr_size_); //receive message from server
...
void SocketServer::ReceiveCloud(char** data, const int data_size)
{
  ...
  for (int i = 0; i < data_size; i += num_bytes) {
    ...
    num_bytes = recvfrom(sock_fdesc_conn_, sock_data + i, data_size - i, 0, 
      (struct sockaddr*)&client_addr_,&client_addr_size_); 
