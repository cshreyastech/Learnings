...
void Application::InitializeLifecycle() {
  ...
  lifecycle_callbacks.on_device_active = [](void *context) {
    ML_LOG(Info, "On active called.");
    Application *this_app = static_cast<Application *>(context);
    this_app->lifecycle_status_ = ApplicationStatus::DeviceActive;
  };
  lifecycle_callbacks.on_device_reality = [](void *context) {
    ML_LOG(Info, "On reality called.");
    Application *this_app = static_cast<Application *>(context);
    this_app->lifecycle_status_ = ApplicationStatus::DeviceReality;
  };
  lifecycle_callbacks.on_device_standby = [](void *context) {
    ML_LOG(Info, "On standby called.");
    Application *this_app = static_cast<Application *>(context);
    this_app->lifecycle_status_ = ApplicationStatus::DeviceStandby;
  };
...
void Application::SyncAppStatus() {
  ...
  if (app_status_ == ApplicationStatus::Running) {
    ...
    OnPause();
  } else if (app_status_ == ApplicationStatus::DeviceStandby) {
    OnDeviceStandby();
  }  else if (app_status_ == ApplicationStatus::DeviceReality) {
    OnDeviceReality();
  } else if (app_status_ == ApplicationStatus::DeviceActive) {
    OnDeviceActive();
  }
}
