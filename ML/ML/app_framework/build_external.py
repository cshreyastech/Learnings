...
def register_args(parser):
    parser.add_argument('--clean', '-c', action='store_true',
                        help='''remove the build output of ext dependencies''')
    parser.add_argument('--rebuild', '-r', action='store_true',
                        help='''clean then build the ext dependencies''')
...
def run_cmake(target, parallel=False):
    ...
    source_path = os.path.join(ABSOLUTE_PATH, 'ext')
    build_path = os.path.join(ABSOLUTE_PATH, 'ext', 'build', mabu_spec)
    install_path = os.path.join(ABSOLUTE_PATH, 'ext', 'package', mabu_spec)
...
def clean():
    base_build_path = os.path.join(ABSOLUTE_PATH, 'ext', 'build')
    base_install_path = os.path.join(ABSOLUTE_PATH, 'ext', 'package')
