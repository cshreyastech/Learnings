...
def layout_package(build_args):
    print('build_project.py. layout_package()-------------build_args', build_args)
...
def build_packages(build_args):
     ...
     # Use relative paths, in order to avoid path limits and command line length limits on windows
     mabu_projects = list(os.path.relpath(p) for p in get_area_projects(build_args))
