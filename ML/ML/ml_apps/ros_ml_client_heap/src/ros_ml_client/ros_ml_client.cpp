...
bool RosMLClient::OnUserUpdate(float fElapsedTime)
{
    ...
    while (!Incoming().empty())
    {
      ...
      switch (msg.header.id)
      {
        ...
        case(GameMsg::Client_AssignID):
        {
          ...
          GameEngine::InitializePointCloud(n_points);
  ...
  // ML_LOG_TAG(Info, APP_TAG, "%f", point_cloud_.points[mapObjects_[nPlayerID_].n_points - 1].Color.v0);
  ...
  GameEngine::PublishCloud(point_cloud_);
